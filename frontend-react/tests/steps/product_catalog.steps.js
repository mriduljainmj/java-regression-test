...
// These step definitions drive the rendered DOM (data-testid hooks) via
// Playwright — so they are identical to the ones used for the plain-HTML UI.
// That is the whole point: UI automation targets the DOM, not the framework.
// Each step logs its action via the shared logger; the After hook screenshots
// on failure.

const rowSelector = (name) => `[data-testid=product-row][data-name="${name}"]`;

Feature: Out-of-stock product count

  As a client of the Product API
  I want to know how many products are currently out of stock
  ...
  Scenario: Count reflects only out-of-stock products
    Given the product catalog is empty
    And a product exists with payload:
      """
      { "Name": "Widget A", "Price": 10.0, "InStock": false }
      """
    And a product exists with payload:
      """
      { "Name": "Widget B", "Price": 15.0, "InStock": true }
      """
    And a product exists with payload:
      """
      { "Name": "Widget C", "Price": 20.0, "InStock": false }
      """
    When a client requests GET /api/products/out-of-stock-count
    Then the response status should be 200

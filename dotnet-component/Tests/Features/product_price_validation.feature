Feature: Product price validation
  ...
  Scenario: Create product with price exceeding maximum returns 400
    When a client POSTs /api/products with body
      """
      { "Name": "Expensive Product", "Price": 2000001, "InStock": true }
      """
    Then the response status should be 400
    And the response JSON should contain message "Product price must not exceed 2000000."

  Scenario: Create product with price equal to maximum allowed succeeds
    When a client POSTs /api/products with body
      """
      { "Name": "Max Price Product", "Price": 2000000, "InStock": true }
      """
    Then the response status should be 201
    And the response JSON should contain "Price": 2000000

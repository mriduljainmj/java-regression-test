Feature: Product management
  ...
  Scenario: Create a product with a price up to the new maximum
    When a client creates a product with name "SuperComputer" and price 99999.99
    Then the response status should be 201
    And the response should contain a product id
  ...
  Scenario Outline: Reject invalid product creation requests
    ...
    Examples:
      ...
      | Laptop | 200001.00  | price must not exceed 200000.00         |
  ...
  Scenario Outline: Reject invalid product update requests
    ...
    Examples:
      ...
      | 1  | Laptop | 200001.00  | price must not exceed 200000.00         |
